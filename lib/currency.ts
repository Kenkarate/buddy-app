...
export const SUPPORTED_CURRENCIES: Record<string, { symbol: string; locale: string }> = {
...
let ratesCache: { data: Record<string, number> | null; fetchedAt: number } = {
  data: null,
  fetchedAt: 0,
};
...
export async function getExchangeRates(): Promise<Record<string, number>> {
  ...
  try {
    ...
    if (response.data?.result === "success" && response.data.rates) {
      ...
      return ratesCache.data as Record<string, number>;
    }

    throw new Error("Unexpected exchange rate response");
  } catch (error) {
    console.warn(
      "Falling back to static exchange rates:",
...
export function getCurrencyForCountry(countryCode?: string): string {
  return (
    COUNTRY_CURRENCY_MAP[String(countryCode || "").toUpperCase()] || BASE_CURRENCY
  );
}
...
export function convertFromInrPaise(
  amountInPaise: number,
  currency: string,
  rates?: Record<string, number>
): { amount: number; displayAmount: number } {
