...
export function extendPlan(
  user: UserDoc,
  { plan, addMonths: months = 1 }: { plan: string; addMonths?: number }
): Date | null {
  const existing = (user.purchasedPlans || []).find(
    (p: { plan: string }) => p.plan === plan
  );
...
// purchase was found.
export function revokePlan(user: UserDoc, { plan }: { plan: string }): boolean {
  const existing = (user.purchasedPlans || []).find(
    (p: { plan: string }) => p.plan === plan
  );
  if (existing) {
...
interface UpsertSubscriptionOpts {
  plan?: string;
...
export function upsertSubscription(
  user: UserDoc,
  {
    ...
    currentEnd,
    cancelledAt,
  }: UpsertSubscriptionOpts
) {
  user.subscriptions = user.subscriptions || [];

  let record =
    user.subscriptions.find(
      (s: { razorpaySubscriptionId?: string }) =>
        s.razorpaySubscriptionId === razorpaySubscriptionId
    ) ||
    (razorpaySubscriptionId
      ? null
      : user.subscriptions.find((s: { plan?: string }) => s.plan === plan));
  ...
  if (razorpaySubscriptionId !== undefined)
    record.razorpaySubscriptionId = razorpaySubscriptionId;
